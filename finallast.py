...
if selected == "Predictions":
    ...
    with st.form('Regression'):
        ...
        if submit_button:
            try:
                with open("singaporeflat_model.pkl", 'rb') as file:
                ...
                # Create input array
                user_data = np.array([
                    month, floor_area_sqm_input, lease_commence_date_input, year,
                    street_name_encoded, town_encoded, flat_type_encoded, flat_model_encoded,
                    block_encoded, lower_bound, upper_bound, price_per_sqm, remaining_lease_input
                ]).reshape(1, -1)
